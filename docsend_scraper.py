...
def main():
    ...
    if url.lower().endswith('.pdf') or '.pdf?' in url.lower() or '/files/blob/' in url.lower():
        ...
        try:
            ...
            print(f"[Scraper] PDF detection error: {e}")
    ...
    try:
        with sync_playwright() as p:
            # Launch browser with dynamic install fallback if binary is missing
            try:
                browser = p.chromium.launch(headless=True)
            except Exception as launch_err:
                print(f"[Scraper] Chromium launch failed: {launch_err}. Attempting dynamic install...")
                import subprocess
                subprocess.check_call([sys.executable, "-m", "playwright", "install", "chromium"])
                # Retry launching
                browser = p.chromium.launch(headless=True)
